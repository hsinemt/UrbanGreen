...
@section('title', 'Taux de Change en Temps Réel')
    ...
    <div class="row">
      <div class="col-lg-8">
        <div class="cs_pl_75">
            ...
            <div class="row cs_gap_y_20">
              ...
              <div class="col-md-3">
                ...
                <select id="convertTo" class="cs_form_select">
                  @foreach($supportedCurrencies as $code => $name)
                    <option value="{{ $code }}" {{ $code == 'TND' ? 'selected' : '' }}>
                      {{ $code }} - {{ $name }}
                    </option>
                  @endforeach
                </select>
          ...
          @if($liveRates && $liveRates['success'])
            <div class="cs_white_bg cs_shadow_1 cs_padding_large">
              ...
              <div class="row cs_mb_20">
                <div class="col-md-6">
                  <p class="cs_mb_0"><strong>Devise de base:</strong> {{ $liveRates['base'] }}</p>
                </div>
                <div class="col-md-6">
                  <p class="cs_mb_0"><strong>Dernière mise à jour:</strong> {{ $liveRates['date'] }}</p>
                </div>
              </div>
              ...
              <div class="table-responsive">
                <table class="cs_table cs_style_1">
                  ...
                  <tbody>
                    @foreach($supportedCurrencies as $code => $name)
                      @if(isset($liveRates['rates'][$code]) || $code == $liveRates['base'])
                        <tr>
                          <td><strong>{{ $code }}</strong></td>
                          <td>{{ $name }}</td>
                          <td>
                            @if($code == $liveRates['base'])
                              <span class="badge bg-primary">Devise de base</span>
                            @else
                              {{ number_format($liveRates['rates'][$code], 4) }}
                            @endif
                          </td>
                          <td>
                            @php
