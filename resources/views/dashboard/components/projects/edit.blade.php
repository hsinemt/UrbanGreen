...
<div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
    <h6 class="fw-semibold mb-0">Edit Project</h6>
    ...
    <div class="col-lg-8 mx-auto">
        <div class="card h-100 p-0 radius-12">
            <div class="card-header border-bottom bg-base py-16 px-24">
                <h4 class="fw-semibold mb-0">Edit Project: {{ $projet->name }}</h4>
            </div>
            <div class="card-body p-24">
                <form action="{{ route('back.projects.update', $projet) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control radius-8 @error('name') is-invalid @enderror" 
                                   name="name" value="{{ old('name', $projet->name) }}" required>
                            @error('name')
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control radius-8 @error('user_id') is-invalid @enderror" name="user_id">
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ old('user_id', $projet->user_id) == $user->id ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                                @if(!$projet->user_id)
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control radius-8 @error('description') is-invalid @enderror" 
                                  name="description" rows="4" required>{{ old('description', $projet->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" class="form-control radius-8 @error('progress_percentage') is-invalid @enderror" 
                                   name="progress_percentage" value="{{ old('progress_percentage', $projet->progress_percentage) }}" 
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" class="form-control radius-8 @error('budget') is-invalid @enderror" 
                                   name="budget" value="{{ old('budget', $projet->budget) }}" 
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" class="form-control radius-8 @error('start_date') is-invalid @enderror" 
                                   name="start_date" value="{{ old('start_date', $projet->start_date->format('Y-m-d')) }}" required>
                            @error('start_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" class="form-control radius-8 @error('end_date') is-invalid @enderror" 
                                   name="end_date" value="{{ old('end_date', $projet->end_date ? $projet->end_date->format('Y-m-d') : '') }}">
                            @error('end_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control radius-8 @error('priority') is-invalid @enderror" name="priority">
                                ...
                                @for($i = 1; $i <= 5; $i++)
                                    <option value="{{ $i }}" {{ old('priority', $projet->priority) == $i ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control radius-8 @error('visibility') is-invalid @enderror" name="visibility">
                                ...
                                <option value="public" {{ old('visibility', $projet->visibility) == 'public' ? 'selected' : '' }}>Public</option>
                                <option value="private" {{ old('visibility', $projet->visibility) == 'private' ? 'selected' : '' }}>Private</option>
                            </select>
                            @error('visibility')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control radius-8 @error('tags') is-invalid @enderror" 
                               name="tags" value="{{ old('tags', is_array($projet->tags) ? implode(', ', $projet->tags) : $projet->tags) }}" 
